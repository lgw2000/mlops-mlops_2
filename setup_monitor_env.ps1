# ====================================================
# Docker 환경 설정 & 실행 스크립트 (PowerShell Ver.)
# ====================================================
# 이 스크립트는 현재 디렉토리(mlops-mlops_2) 내에 독립적인 모니터링 환경
# (Grafana, Prometheus, StatsD)을 설정합니다.

Write-Host "--- [1] 디렉토리 구조 생성 ---"
New-Item -ItemType Directory -Force -Path docker-data/prometheus/config | Out-Null
New-Item -ItemType Directory -Force -Path docker-data/prometheus/data | Out-Null
New-Item -ItemType Directory -Force -Path docker-data/grafana/data | Out-Null
New-Item -ItemType Directory -Force -Path docker-data/grafana/provisioning/datasources | Out-Null
New-Item -ItemType Directory -Force -Path docker-data/grafana/provisioning/dashboards | Out-Null
New-Item -ItemType Directory -Force -Path docker-data/grafana/dashboards | Out-Null
New-Item -ItemType Directory -Force -Path docker-data/statsd | Out-Null

Write-Host "--- [2] 설정 파일 생성 ---"

# 2.0 Grafana 프로비저닝
Write-Host "Grafana 프로비저닝 설정 생성 중..."
...
datasources:
  - name: Prometheus
    ...
    isDefault: true
'@
Set-Content -Path docker-data/grafana/provisioning/datasources/datasource.yml -Value $datasourceContent -Encoding UTF8
...
'@
Set-Content -Path docker-data/grafana/provisioning/dashboards/dashboard.yml -Value $dashboardProviderContent -Encoding UTF8

# 대시보드 JSON (임베디드)
Write-Host "대시보드 JSON 생성 중..."
$dashboardJsonContent = @'
{
  "annotations": {
    "list": [
      {
...
'@
Set-Content -Path docker-data/grafana/dashboards/model_monitor_dashboard.json -Value $dashboardJsonContent -Encoding UTF8

# 2.1 Prometheus 설정
Write-Host "docker-data/prometheus/config/prometheus.yml 생성 중..."
$prometheusContent = @'
global:
  scrape_interval: 15s
...
'@
Set-Content -Path docker-data/prometheus/config/prometheus.yml -Value $prometheusContent -Encoding UTF8

# 2.2 StatsD 설정
Write-Host "docker-data/statsd/statsd.conf 생성 중..."
$statsdContent = @'
mappings:
  ...
  - match: "model\\.drift\\.wasserstein\\.(.+)"
    ...
    labels:
      feature: "$1"
'@
Set-Content -Path docker-data/statsd/statsd.conf -Value $statsdContent -Encoding UTF8

Write-Host "--- [3] docker-compose.yml 생성 ---"
...
Write-Host "--- [4] Docker 컨테이너 시작 ---"
docker-compose up -d

Write-Host "--- 설정 완료! ---"
Write-Host "Grafana: http://localhost:3000"
Write-Host "Prometheus: http://localhost:9090"
Write-Host "StatsD Exporter metrics: http://localhost:9102/metrics"
